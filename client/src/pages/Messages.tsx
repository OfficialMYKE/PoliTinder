...
import {
  ...
  subscribeToMessages, subscribeToConversations, getOtherParticipant,
} from "../services/messages"
import type { ConversationWithLastMessage, MessageWithProfile } from "../types/message"
import {
  MessageSquare, Search, Send, ArrowLeft, Loader2, CheckCheck, Edit3, X,
  MoreVertical, Video, Phone, Paperclip, Smile, Archive, UserPlus, Check, XCircle,
} from "lucide-react"
...
import {
  ...
  getArchivedConversationIds, archiveConversation, unarchiveConversation,
  markConversationRead, hasUnreadMessages, getReadColumn,
...
export default function Messages() {
  ...
  const [userResults, setUserResults] = useState<{ id: string; nickname: string; avatar_url: string | null }[]>([])
  ...
  useEffect(() => {
    if (!currentUserId || tab !== "solicitudes") { setIncomingRequests([]); return }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      const { data } = await supabase
        ...
        .limit(10)
      setUserResults((data ?? []) as { id: string; nickname: string; avatar_url: string | null }[])
  ...
  // ── Select conversation ──
  const selectConversation = useCallback(async (conv: ConversationWithLastMessage) => {
    ...
    setMessages(msgs)
    if (currentUserId) {
      markConversationRead(conv.id, currentUserId, getReadColumn(currentUserId, conv))
    }
  }, [currentUserId])

  // ── Send message ──
  async function handleSend() {
    if (!selectedConv || !currentUserId || !messageText.trim() || sending) return
    setSendingError("")
    setSending(true)
  ...
  return (
    <div className="flex h-[calc(100vh-0px)]">
      {/* ── Conversation list ── */}
      <div className={`w-full md:w-80 lg:w-96 border-r border-zinc-200 dark:border-zinc-800 flex flex-col bg-white dark:bg-zinc-950 ${selectedConv ? "hidden md:flex" : "flex"}`}>
        <header className="sticky top-0 z-10 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md border-b border-zinc-200 dark:border-zinc-800 px-4 py-4">
          <div className="flex items-center justify-between mb-3">
            <h1 className="text-lg font-semibold text-zinc-900 dark:text-white">
              {showNewChat ? "Nuevo chat" : tab === "archivados" ? "Archivados" : tab === "solicitudes" ? "Solicitudes" : "Mensajes"}
            </h1>
            <button
              type="button"
              onClick={() => { setShowNewChat(!showNewChat); setUserSearch(""); setUserResults([]) }}
            ...
            >
              {showNewChat ? <X className="w-4 h-4" /> : <Edit3 className="w-4 h-4" />}
            </button>
          </div>
        ...
        {showNewChat && (
          <div className="flex-1 overflow-y-auto">
            {userSearchLoading ? (
              <div className="flex items-center justify-center py-12"><Loader2 className="w-5 h-5 text-zinc-400 animate-spin" /></div>
              ...
              <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
                {userResults.map((u) => (
                  ...
                  >
                    ...
                    <span className="text-sm font-medium text-zinc-900 dark:text-white">{u.nickname}</span>
                  </button>
                ))}
              </div>
            ) : (
              <div className="flex flex-col items-center text-center px-6 py-12">
                ...
                <p className="text-sm text-zinc-500">Busca usuarios para iniciar un chat</p>
              </div>
            )}
          </div>
        )}
          ...
          <div className="flex-1 overflow-y-auto">
            {requestsLoading ? (
              ...
              <div className="flex flex-col items-center text-center px-6 py-20">
                ...
                <h2 className="text-xl font-semibold text-zinc-900 dark:text-white mb-2">Sin solicitudes</h2>
                <p className="text-sm text-zinc-500">No tienes solicitudes de mensaje pendientes.</p>
              </div>
            ) : (
              <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
                {incomingRequests.map((req) => (
                  <div key={req.id} className="px-4 py-3.5 flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full overflow-hidden bg-[#487CFF]/10 shrink-0 flex items-center justify-center text-sm font-semibold text-[#487CFF]">
                      {req.sender_avatar ? (
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center text-center px-6 py-20">
                ...
                <h2 className="text-xl font-semibold text-zinc-900 dark:text-white mb-2">
                  {search ? "Sin resultados" : tab === "archivados" ? "Sin chats archivados" : "Sin conversaciones"}
                </h2>
              ...
              <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
                {filtered.map((conv) => {
                  ...
                  const isLastFromMe = conv.last_message_sender_id === currentUserId
                  return (
                    <button
                      key={conv.id}
                      onClick={() => selectConversation(conv)}
                      className={`w-full text-left px-4 py-3.5 hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors ${
                        selectedConv?.id === conv.id ? "bg-zinc-100 dark:bg-zinc-800" : ""
                      }`}
                    >
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            {conv.last_message_created_at && (
                              <span className="text-[10px] text-zinc-400 shrink-0">
                                {new Date(conv.last_message_created_at).toLocaleTimeString("es-MX", {
                          ...
                          {conv.last_message_content && (
                            <span className="text-xs text-zinc-500 dark:text-zinc-400 block truncate">
                              {isLastFromMe && <CheckCheck className="w-3 h-3 inline mr-0.5 -mt-0.5" />}
                              {conv.last_message_content}
                            </span>
                          )}
                        </div>
      ...
      {/* ── Chat view ── */}
      <div className={`flex-1 flex flex-col bg-white dark:bg-zinc-950 ${!selectedConv ? "hidden md:flex" : "flex"}`}>
        {selectedConv && other ? (
          <>
            ...
            <header className="sticky top-0 z-10 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md border-b border-zinc-200 dark:border-zinc-800 px-4 py-3 flex items-center gap-3">
              <button type="button" onClick={goBack} className="md:hidden p-1 -ml-1 text-zinc-600 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-white">
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button type="button" onClick={() => initiateCall('video')} className="p-2 rounded-full text-zinc-500 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-zinc-700 dark:hover:text-white transition-colors cursor-pointer">
                  <Video className="w-5 h-5" />
                </button>
                <button type="button" onClick={() => initiateCall('voice')} className="p-2 rounded-full text-zinc-500 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-zinc-700 dark:hover:text-white transition-colors cursor-pointer">
                    ...
                    >
                      {archivedIds.includes(selectedConv.id) ? (
                        <button
                          type="button"
                          onClick={async () => {
                            await unarchiveConversation(selectedConv.id, currentUserId!)
                            setArchivedIds((prev) => prev.filter((id) => id !== selectedConv.id))
                            setHeaderMenuOpen(false)
                          }}
                        ...
                        <button
                          type="button"
                          onClick={async () => {
                            await archiveConversation(selectedConv.id, currentUserId!)
                            setArchivedIds((prev) => [...prev, selectedConv.id])
                            setHeaderMenuOpen(false)
                          }}
                          className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors cursor-pointer"
                        >
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1">
              {messages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center px-6">
                  <p className="text-sm text-zinc-400">No hay mensajes aún. Envía un saludo.</p>
                </div>
              ) : (
                messages.map((msg, idx) => {
                  ...
                  return (
                    <div key={msg.id}>
                      ...
                      <div className={`flex ${isMine ? "justify-end" : "justify-start"} ${isFirstOfGroup ? "mt-2" : "mt-0.5"}`}>
                        <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
                          ...
                          {msg.reply_to_story_id && (
                            <div className={`mb-2 rounded-lg p-2 ${isMine ? "bg-[#3a6ae0]" : "bg-zinc-200 dark:bg-zinc-700"}`}>
          ...
          <div className="flex-1 flex items-center justify-center px-6">
            <div className="flex flex-col items-center text-center max-w-sm">
              <MessageSquare className="h-12 w-12 text-zinc-300 dark:text-zinc-600 mb-4" />
              <h2 className="text-xl font-semibold text-zinc-900 dark:text-white mb-2">Selecciona una conversación</h2>
              <p className="text-sm text-zinc-500">Elige un chat de la lista para ver los mensajes.</p>
            </div>
          </div>
        )}
      </div>
